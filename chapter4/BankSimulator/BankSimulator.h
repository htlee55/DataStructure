...
class BankSimulator
{
	...
	int	nCustomers;				// 현재까지 도착한 고객 수
	int	nServedCustomers;		// 서비스 받은 고객 수
	CustomerQueue	que;		// 대기 고객 큐
...
public:
	...
	void readSimulationParameters() {
		...
		scanf_s("%d", &nSimulation, sizeof(nSimulation));
		printf("단위시간에 도착하는 고객 수 (예:0.5) = ");
		scanf_s("%lf", &probArrival, sizeof(probArrival));
		printf("한 고객에 대한 최대 서비스 시간 (예:5) = ");
		scanf_s("%d", &tMaxService, sizeof(tMaxService));
		printf("====================================================\n");
	}
